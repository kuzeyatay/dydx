...
async def place_market_order(client, market, side, size, price, reduce_only):
    ...
    # Set Time In Force
    time_in_force = Order.time_in_force.TIME_IN_FORCE_IOC
    ...
    order = await client.node.place_order(
        ...
        market.order(
            ...
            size=float(size),
            price=0,  # Using 0 for MARKET orders
    ...
    for order in orders:
        ...
        # Safely access 'createdAtHeight'
        created_at_height = order.get("createdAtHeight")
        if created_at_height is not None:
            order["createdAtHeight"] = int(created_at_height)
    ...
    if order_id == "":
        ...
        print("last order:")
        pprint(sorted_orders[0])  # Corrected pprint usage
